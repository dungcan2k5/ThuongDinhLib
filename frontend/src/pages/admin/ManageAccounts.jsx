...
const ManageAccounts = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${getApiUrl()}/api/staffs/${selectedStaff._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
              ...
              {Array.isArray(staffs) && staffs.length > 0 ? (
                staffs.map((staff) => (
                  <tr key={staff._id}>
                    ...
                    <td>
                      <button className="acc-edit-btn" onClick={() => handleEdit(staff)}>
                        <FontAwesomeIcon icon={faPenToSquare} />
                      </button>
                      <button className="acc-delete-btn" onClick={() => handleDelete(staff._id, staff.isAdmin)}>
        ...
        <div className="acc-modal-overlay">
          <div className="acc-modal">
            <div className="acc-modal-header">
              ...
              <button className="acc-close-btn" onClick={() => setCreateOpen(false)}>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div className="acc-modal-footer">
                <button type="button" className="acc-cancel-btn" onClick={() => setCreateOpen(false)}>Hủy</button>
                <button 
                  type="submit" 
      ...
      {open && (
        <div className="acc-modal-overlay">
          <div className="acc-modal">
            ...
            <div className="acc-modal-footer">
              <button className="acc-cancel-btn" onClick={handleClose}>Hủy</button>
              <button className="acc-save-btn" onClick={handleSubmit}>Lưu</button>
            </div>
          </div>
        </div>
      )}
