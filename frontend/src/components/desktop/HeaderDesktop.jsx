...
import { searchBooks, getBaseURL } from "../../services/bookService"; // Đảm bảo import đúng đường dẫn
        ...
        <div className="header">
            ...
            <div className="header__center">
                ...
                >
                    ...
                    />
                    {isFocused && (<div className={'header__search-dropdown'} style={{ display: results.length === 0 ? 'none' : 'block' }}>
                        {results.map((book) => (
                            <div key={book._id} className="header__search-item">
                                <div>
                                    <img src={`${getBaseURL()}${book.image}`} alt={book.title} width={50} height={75} />
                                    <div>
                                        <strong>{book.title}</strong>
                                        <div style={{ color: '#E9A319' }}>{book.author}</div>
                                        <div style={{marginTop: '16px'}}>{book.price.toLocaleString('vi-VN')} đ</div>
                                    </div>
                                </div>
                            </div>
                        ))}
